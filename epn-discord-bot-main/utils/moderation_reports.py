...
class ModerationReportView(discord.ui.View):
    """Clean moderation report view with persistent buttons."""
    
    def __init__(self, report_id: str, timeout: float = 86400):
        super().__init__(timeout=timeout)
        self.report_id = report_id
        
        # Add custom IDs to buttons for persistence
        for item in self.children:
            if isinstance(item, discord.ui.Button):
                if item.label == "Accept Report":
                    item.custom_id = f"mod_accept_{report_id}"
                elif item.label == "Deny Report":
                    item.custom_id = f"mod_deny_{report_id}"
                elif item.label == "View Message":
                    item.custom_id = f"mod_view_{report_id}"

    @discord.ui.button(label="Accept Report", style=discord.ButtonStyle.danger)
    async def accept_report(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not await self._check_staff_permission(interaction):
            await interaction.response.send_message("! Only staff members can accept moderation reports.", ephemeral=True)
            return
    ...
    @discord.ui.button(label="Deny Report", style=discord.ButtonStyle.secondary)
    async def deny_report(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Deny the moderation report."""
        if not await self._check_staff_permission(interaction):
            await interaction.response.send_message("! Only staff members can deny moderation reports.", ephemeral=True)
            return
    ...
    async def _check_staff_permission(self, interaction: discord.Interaction) -> bool:
        ...
        try:
            ...
            # Check if user is staff using the StaffUtils
            if StaffUtils.is_staff(interaction.user):
                return True
            
            # Check if user has developer permission (cross-guild)
            if await StaffUtils.has_developer_permission_cross_guild(interaction.client, interaction.user):
                return True
...
class ModerationReportManager:
    ...
    async def send_moderation_report(self, scan_data: Dict[str, Any], message: discord.Message) -> Optional[discord.Message]:
        ...
        try:
            # Use the specific AI reports channel ID
            report_channel_id = 1413676568383066142
            
            report_channel = self.bot.get_channel(report_channel_id)
            if not report_channel:
                logger.error(f"AI reports channel not found (ID: {report_channel_id})")
                return None
            ...
            # Create a thread for staff discussion
            try:
                thread_name = f"AI Report - {message.author.display_name} - {message.id}"
                # Limit thread name length to 100 characters (Discord limit)
                if len(thread_name) > 100:
                    thread_name = f"AI Report - {message.author.display_name[:50]} - {message.id}"
                
                thread = await report_message.create_thread(name=thread_name)
                
                # Send initial message in thread
                thread_embed = discord.Embed(
                    title="Staff Discussion Thread",
                    description="Use this thread to discuss the AI moderation report above.",
                    color=0x3498db
                )
                ...
                thread_embed.set_footer(text="This thread is for staff discussion only")
                
                await thread.send(embed=thread_embed)
                
                logger.info(f"Created staff discussion thread for report {report_id}")
                
            except Exception as e:
                logger.error(f"Error creating staff discussion thread: {e}")
